...
export default function InventoryEditDrawer({ open, onClose, item, onSave }) {
  ...
  useEffect(() => {
    if (item) {
      setForm({
        name: item.name || item.product?.productName || '',
        serial: item.serial || '',
        category: item.category || item.product?.category || '',
        unit: item.unit || item.product?.unit || '',
        price: item.price || item.product?.price || '',
        location: item.location || item.location?.locationName || '',
        stock: item.stock || item.onHand || '',
